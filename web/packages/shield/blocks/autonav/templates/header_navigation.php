...
$colors = array('darkblue', 'grey', 'green', 'earth', 'lightblue');
$colorIndexStart = 0;
...
foreach ($navItems as $ni) {
    ...
    // modification to use the 5 preset background colors for snake river k9
    if( $ni->level == 1 ){
        $classes[] = $colors[ $colorIndexStart++ ];
        $colorIndexStart = ($colorIndexStart > 4) ? 0 : $colorIndexStart;
    }
...
echo '<section class="top-bar-section primary-nav-section show-for-large-up"><ul class="left" role="navigation">';
...
echo '</ul></section>'; //closes the top-level menu
