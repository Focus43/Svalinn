...
echo '<ul class="small-block-grid-1 medium-block-grid-2 large-block-grid-4 private-nav">'; //opens the top-level menu

foreach ($navItems as $ni) {
	$img = $ni->cObj->getAttribute('masthead_image');
	
	echo '<li class="' . $ni->classes . '">'; //opens a nav item

	//echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . $ni->name . '</a>';
    echo '<a ' . sprintf('href="%s"', $ni->url) . '">';
    if ($img) {
		  $thumb = $ih->getThumbnail($img, 350, 200, true); //<--set thumbnail width/height here, and change FALSE to TRUE if you want the thumbnail cropped
